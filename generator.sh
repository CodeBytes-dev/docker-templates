...
readonly script_author="yair.miranda@tec.mx"
...
option|l|log_dir|where log files are stored|$HOME/log/$script_prefix
option|t|tmp_dir|where temporary files are stored|.tmp
flag|n|no-git|explain the 'no-git' flag
flag|d|no-docker|explain the 'no-docker' flag
flag|p|no-install|explain the 'no-install' flag
option|g|git-url|explain the 'git-url' option|def_git-url
param|1|action|explain the 'action' param
" | grep -v '^#' | grep -v '^\s*$'
}
...
main() {
	...
	#// the flag $no-git will be 0 or 1
	#// the flag $no-docker will be 0 or 1
	#// the flag $no-install will be 0 or 1
	#// the option $git-url will be [def_git-url] or whatever the user specified with -g [value] or --git-url [value]
